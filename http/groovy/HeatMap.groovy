...
public class HeatMap extends Resource {
	...
	// Function to get the Max value in Array
        colorCells('heat-map-0');
        colorCells('heat-map-1');
        colorCells('heat-map-2');
});"""
                }
                style(type:'text/css') {
                    mkp.yieldUnescaped """
...
"""
            ...
            body() {
                def wavePerks= reader.executeQuery("server_match_wave_perks", matchUUID).findAll{it.wave == 4}
                def data= reader.executeQuery("server_match_wave_data", matchUUID).findAll{it.wave == 4}
                def index= -1
                        ...
                        tbody() {
                            data.findAll{it.category == category}.collect(new HashSet()){ it.stat }.each{ stat ->
                                tr(class:'stats-row') {
                                    ...
                                    wavePerks.each {perk ->
                                        def result= data.find{it.stat == stat && it.perk == perk.name && it.category == category}
